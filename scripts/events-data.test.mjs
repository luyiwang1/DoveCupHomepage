...
test('keeps a bilingual event album ready for real activity photos', () => {
  assert.equal(event.eventNumber, 10);
  ...
  assert.equal(event.gallery.photos[0].caption, 'EVENT 10 · 8 月 8 日凤凰 vs 狮鹫团体赛 · MRTC 赛后合影');
  assert.equal(event.gallery.photos[0].captionEn, 'EVENT 10 · Aug 8 Phoenix vs Griffin team event · Post-match photo at MRTC');
});
