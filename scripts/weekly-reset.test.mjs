...
test('finds the latest Saturday 23:00 cutoff in Toronto across DST', () => {
  assert.equal(latestEligibleResetId(new Date('2026-08-02T02:59:00Z')), '2026-07-25');
  assert.equal(latestEligibleResetId(new Date('2026-08-02T03:00:00Z')), '2026-08-01');
  assert.equal(latestEligibleResetId(new Date('2026-12-06T03:59:00Z')), '2026-11-28');
  assert.equal(latestEligibleResetId(new Date('2026-12-06T04:00:00Z')), '2026-12-05');
});
...
test('archives both lists, counts joined players once, and clears the signup form', () => {
  ...
  assert.equal(result.data.signupHistory['2026-08-01'].scheduledFor, '2026-08-01T23:00:00[America/Toronto]');
  assert.equal(result.data.weeklyReset.archiveHour, 23);
